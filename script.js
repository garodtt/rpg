...
function addArma(){
  ...
  aplicarEventosArma(li); // 🔥 ÚNICA fonte de eventos
  document.getElementById("listaArmas").appendChild(li);
...
function preencherFormularioComFicha(f){
    ...
    if (Array.isArray(f.inventario) && f.inventario.some(i => i.cavalo)) {
      cavaloAtivo = true;
      pesoCavaloMax = f.tipo_cavalo ?? 30;
      selectTipoCavalo.value = String(pesoCavaloMax);
      boxCavalo.style.display = "block";
    ...
    if (Array.isArray(f.arma)) {
      f.arma.forEach(a => {
        ...
        document.gaplicarEventosArma(li);
        document.getElementById("listaArmas").appendChild(li);
      });
